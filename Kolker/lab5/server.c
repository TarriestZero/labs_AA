...
int main() {
    ...
    server_address.sin_family = AF_INET;   ///интернет домен
    server_address.sin_addr.s_addr = INADDR_ANY;   ///соединяемся с 127,0,0,1
    server_address.sin_port = htons(56000);    /// 21 порт
    ...
    while(1) {
        ...
        for (int i = 0;i<10;i++) printf("%d",ch[i]);
        printf("  ------ прием\n");
